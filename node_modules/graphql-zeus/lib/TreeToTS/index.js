...
class TreeToTS {
    ...
    static resolveBasisCodeJavascript(tree) {
        ...
        const returnTypes = `export const ReturnTypes = {\n${tree.nodes
            .map((f) => returnedReturns_1.resolveReturnFromRoot(f, f.data.type === Models_1.TypeDefinition.InterfaceTypeDefinition
    ...
    static resolveBasisTypes(tree) {
        const rootTypes = returnedTypes_1.resolveTypes(tree.nodes);
        const valueTypes = resolveValueTypes_1.resolveValueTypes(tree.nodes);
        const modelTypes = returnedModelTypes_1.resolveModelTypes(tree.nodes);
        const unionTypes = returnedTypes_1.resolveUnions(tree.nodes);
        const interfaceTypes = returnedTypes_1.resolveInterfaces(tree.nodes);
    ...
    static javascriptSplit(tree, env = 'browser', host) {
        ...
        const operations = javascript_1.bodyJavascript(env, operationsBody);
        return {
            ...
            definitions: TreeToTS.resolveBasisTypes(tree)
                ...
                .concat(javascript_1.generateOperationsJavascript(operationsBody)),
        };
    }
    static resolveTreeSplit(tree, env = 'browser', host) {
        const operations = typescript_1.bodyTypeScript(env, TreeToTS.resolveOperations(tree));
        return {
            indexImports: `import { AllTypesProps, ReturnTypes } from './const';`,
            ...
            index: TreeToTS.resolveBasisTypes(tree)
                ...
                .concat(typescript_1.typescriptFunctions(env))
                .concat(operations)
                .concat(host ? '\n\n' : '')
                .concat(host ? `export const Gql = Chain('${host}')` : ''),
        };
