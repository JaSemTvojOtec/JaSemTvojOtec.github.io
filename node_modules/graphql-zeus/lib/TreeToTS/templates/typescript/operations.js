...
const generateOperationZeus = (t, ot) => `${ot}: (o:${resolveValueTypes_1.VALUETYPES}["${t.name}"]) => queryConstruct('${ot}', '${t.name}')(o)`;
...
const generateOperationCast = (t, ot) => `${ot}: ((o: any) => (_: any) => o) as CastToGraphQL<
  ${resolveValueTypes_1.VALUETYPES}["${t.name}"],
  ${returnedTypes_1.TYPES}["${t.name}"]
>`;
const generateOperationsCastTypeScript = ({ query, mutation, subscription }) => {
    var _a, _b, _c;
    const allOps = [];
    if (((_a = query === null || query === void 0 ? void 0 : query.operationName) === null || _a === void 0 ? void 0 : _a.name) && query.operations.length) {
        allOps.push(generateOperationCast(query.operationName, Models_1.OperationType.query));
    }
    if (((_b = mutation === null || mutation === void 0 ? void 0 : mutation.operationName) === null || _b === void 0 ? void 0 : _b.name) && mutation.operations.length) {
        allOps.push(generateOperationCast(mutation.operationName, Models_1.OperationType.mutation));
    }
    if (((_c = subscription === null || subscription === void 0 ? void 0 : subscription.operationName) === null || _c === void 0 ? void 0 : _c.name) && subscription.operations.length) {
        allOps.push(generateOperationCast(subscription.operationName, Models_1.OperationType.subscription));
    }
    return allOps;
};
const bodyTypeScript = (env, resolvedOperations) => `
export const Thunder = (fn: FetchFunction, subscriptionFn: SubscriptionFunction) => ({
  ${exports.generateOperationsThunder(resolvedOperations).join(',\n')}
});

export const Chain = (...options: chainOptions) => ({
  ${exports.generateOperationsChaining(resolvedOperations).join(',\n')}
});
export const Zeus = {
