...
Parser.importSchema = (schema) => graphql_1.buildASTSchema(graphql_1.parse(schema));
Parser.documentDefinitionToSerializedNodeTree = (d) => {
    if (graphql_1.isTypeSystemDefinitionNode(d) || graphql_1.isTypeSystemExtensionNode(d)) {
        if ('name' in d) {
            return {
                name: d.name.value,
...
Parser.parse = (schema, excludeRoots = [], libraries = '') => {
    ...
    try {
        parsedSchema = graphql_1.parse(compiledSchema);
    }
    catch (error) {
        console.error(error);
    }
